...
const int safetyFactorOffset = flagDetectionSteps * 4;
...
//Timer variables
int timerTime = 0; //Used for keeping track of the last timer reset
const int waitTime = 5000; //The default time to wait
boolean sendIdleOnce = true;
boolean sendBusyOnce = true;
boolean sendOtherOnce = true;
...
void loop()
{
  ...
  switch (mainCommand)
  {
    ...
    case idle:
      ...
      //intervalSendState(inState);
      turnOnBrakeDisableMotors();
      //Sends the idle status just once
     if(sendIdleOnce)
      {
        sendInt(inState);
        sendIdleOnce = false;
