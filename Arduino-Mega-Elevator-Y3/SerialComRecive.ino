...
void receiveSerialEvent() {
  ...
  if (Serial.available() > 0) {
    if (Serial.find(DEVICE_NAME)) {
      ...
      // The calib param contains in state,  maxPos.zLeft and maxPos.zRigth + the pos of each tray found. (Left and right)
      // Therefor we need an array of 1(instate) +  2(maxPos) + 2*(numberOftrays)
      int arrayLength = (3 + (trayReg.nrOfTrays * 2));
